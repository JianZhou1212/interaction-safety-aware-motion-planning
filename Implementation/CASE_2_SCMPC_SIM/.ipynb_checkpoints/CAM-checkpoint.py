...
class CAM( ):
    ...
    def Constant_Acc(self, x0, N_S): # state fusion steps of IMM-KF & define the primary reference speed
        """"
        Input: 
                    N_S is the number of steps, x0 is the initial state
        Output: 
                    x_state is the modeled state for the horizon N_S
        """
        Ts = self.Ts
        acc = self.acc
        DSV = self.DSV
        ...
        for i in range(N_S+1):
            if i == 0:
                ...
                x_state[3:5, i] = x_state[3:5, i-1] # lateral motion
                x_state[2, i] = acc
    ...
    def Final_Return(self, k, X_Hat, Y, car_index):
        Ts = self.Ts
        H = self.H
        N = self.N
        N_M = self.N_M
        ...
        x_hat_k = list( )
        x_hat_k_1 = X_Hat[k - 1][car_index] # level 3, list, ist, the estimate of last step for fusion
        ...
        for i in range(N_M):
            if mu_k[i] == 0: # if x_hat_k[i] does not exist
                x_hat_k.append(None)
            else:
        ...
        return REF_Speed[m_k], L_Center[2], REF_Speed, mu_k, m_k, x_hat_k, p_k, x_state_k, x_pre_k, y_k_plus_1, x_po_all_k
