...
class IAIMM_KF( ):
    ...
    def ProjectSpeed(self, Obst_k, x_hat_k, RefPrim, car_index):
        ...
        for i in range(N_M): # output is longitudinal position of obstacle on each lane, column
            if np.sum(x_hat_k[i]) == None: # x_hat_k[i] does not exist
                ...
                SEL = np.diag(SEL)
                X_SV = np.array(X_SV) # np.array([infinity]*N, dtype = float) # longitudinal position of DV
    ...
    def Final_Return_Simulator(self, k, MU, X_Hat, P, Y, Obst_k, car_index):
        Ts = self.Ts
        N = self.N
        N_M = self.N_M
        L_Width = self.L_Width
    ...
    def Final_Return_Predictor(self, k, MU, X_Hat, P, Y, Obst_k, car_index):
        Ts = self.Ts
        N = self.N
        N_M = self.N_M
        L_Width = self.L_Width
    ...
    def Sel_Matrix_Diag(self, K_Lat, initial_y, y_SV, m):
        """"
        Input: 
                   initiai_y is an array about the lateral initial state, y_SV is an array about the predicted lateral position of SV, m is a pure model
        Output: 
