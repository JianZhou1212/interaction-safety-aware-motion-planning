...
class IAIMM_KF_MPC( ):
    ...
    def LaneTracking(self, initial_y, m): # lane tracking model
        """"
        Input: 
                   initial_y is an array about the initail state of lateral motion, m is a pure number
        Output: 
        ...
        return Y[0, :] # returns longitudinal position, a row
    ...
    def GMMObstacleRealization(self, Obst_k, MU_k, X_Po_All_k, X_Var_k, Y_Var_k, x_EV_k, epsilon):
        N_Lane = self.N_Lane
        N_Car = self.N_Car
        N_M = self.N_M
        N = self.N
            ...
            if (np.sum(Obst_k[i]) != None) and (Obst_k[i][0, 0] >= x_EV_k): # there is an obstacle for vehicle i
                if (epsilon != 1) and (i != 3):
                    ...
                    for h in range(N + 1): # h is the number of time points along the trajectory
                        ...
                        gmm_k = self.GMM_Model(x_nom_vec, y_nom_vec, x_var_vec, y_var_vec, model_pro, leng_vec, epsilon)
                        GMM_Step_SV[:, h] = gmm_k
                    ...
                    for j in range(N + 1):
                        gmm_k = np.array([temp_x_po[j], temp_y_po[j], 0, 0]) # x-center, y-center, x-width, y-width
                        GMM_Step_SV[:, j] = gmm_k
                        max_y = gmm_k[1] + gmm_k[3] + w_veh*zeta_w # maximum y boundary
                        min_y = gmm_k[1] - gmm_k[3] - w_veh*zeta_w # minimum y boundary
                        max_y_mark[j] = self.LookLane(max_y)
                        mid_y_mark[j] = self.LookLane(gmm_k[1])
    ...
    def GMM_Model(self, x_nom_vec, y_nom_vec, x_var_vec, y_var_vec, model_pro, leng_vec, epsilon):
        w_veh = self.w_veh
        l_veh = self.l_veh
        zeta_w = self.zeta_w
        zeta_l = self.zeta_l
        ...
        hx = 0.55 # x grid (important parameter)
        hy = 0.15 # y grid (important parameter)
    ...
    def ProjectSpeed(self, Obst_k, y_k, x_hat_k, RefPrim, X_Po_All_k, MU_k, X_Var_k, Y_Var_k, epsilon):
        ...
        GMM_Horizon_SV, X_DV_Lane = self.GMMObstacleRealization(Obst_k, MU_k, X_Po_All_k, X_Var_k, Y_Var_k, y_k[0], epsilon) 
    ...
    def Final_Return(self, k, state_k_loc, state_k_glo, Obst_k, y_k, X_Po_All_k, MU_k, X_Var_k, Y_Var_k, epsilon):
        Ts = self.Ts
        N = self.N
        N_M_EV = self.N_M_EV
        N_Lane = self.N_Lane
        ...
        # define the state and primary reference for each mode of EV
        ...
        # update the primary reference speed by interaction-aware optimization (collision-avoidance motivated)
        REF, X_DV_Lane, GMM_Horizon_SV_k = self.ProjectSpeed(Obst_k, y_k, x_hat_k, RefPrim, X_Po_All_k, MU_k, X_Var_k, Y_Var_k, epsilon) 
        ...
        return RefSpeed, L_Center[m_k], mu_k, m_k, x_hat_k, x_pre_k, Traj_k, U_k, state_k_plus_1_loc, state_k_plus_1_glo, y_k_plus_1, GMM_Horizon_SV_k, REF
    
    def Final_Return_Com_Time(self, k, state_k_loc, state_k_glo, Obst_k, y_k, X_Po_All_k, MU_k, X_Var_k, Y_Var_k, epsilon):
        Ts = self.Ts
        N = self.N
        N_M_EV = self.N_M_EV
        N_Lane = self.N_Lane
        ...
        state_k_plus_1_glo = x_pre_k[:, 1]
        y_k_plus_1 = H@state_k_plus_1_glo
        return RefSpeed, L_Center[m_k], mu_k, m_k, x_hat_k, x_pre_k, Traj_k, U_k, state_k_plus_1_loc, state_k_plus_1_glo, y_k_plus_1, GMM_Horizon_SV_k, REF, ocp_time
    ...
    def vehicle_model(self, w, snap, alpha):
        """Vehicle model describing how the variables evolve for a certain input
        Input:
            w = [x, y, phi, v, a, j, delta, omiga]
            longitudinal position, lateral position, heading angle, longitudinal speed, longitudinal acceleration, longitudinal jerk,  
            front tire angle, angular acceleration
            
            snap: longitudinal control input
            alpha: lateral control input
            
        Output:
            Casadi vector of Time derivative the variables
        """
        l_f = self.l_f
        l_r = self.l_r
